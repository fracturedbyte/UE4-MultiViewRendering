...
ENGINE_API void GenerateInstancedStereoCode(FString& Result)
{
	...
	const FShaderParametersMetadata* InstancedView[2] = { nullptr };
	for (TLinkedList<FShaderParametersMetadata*>::TIterator StructIt(FShaderParametersMetadata::GetStructList()); StructIt; StructIt.Next())
	{
		if (!InstancedView[0] && StructIt->GetShaderVariableName() == FString(TEXT("InstancedView")))
		{
			InstancedView[0] = *StructIt;
		}
		else if (!InstancedView[1] && StructIt->GetShaderVariableName() == FString(TEXT("InstancedView2")))
		{
			InstancedView[1] = *StructIt;
		}
	}
	checkSlow(InstancedView[0] != nullptr && InstancedView[1] != nullptr);
	const TArray<FShaderParametersMetadata::FMember>& StructMembers = InstancedView[0]->GetMembers();
	// FB Bulgakov End
	...
	// FB Bulgakov Begin - Instanced Multi View Rendering
	// GetInstancedView2 definition
	Result += "ViewState GetInstancedView2()\r\n";
	Result += "{\r\n";
	Result += "\tViewState Result;\r\n";
	for (int32 MemberIndex = 0; MemberIndex < StructMembers.Num(); ++MemberIndex)
	{
		const FShaderParametersMetadata::FMember& Member = StructMembers[MemberIndex];
		Result += FString::Printf(TEXT("\tResult.%s = InstancedView2.%s;\r\n"), Member.GetName(), Member.GetName());
	}
	Result += "\treturn Result;\r\n";
	Result += "}\r\n";
	// FB Bulgakov End
