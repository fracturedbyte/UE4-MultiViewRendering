			...
			if (ClusterTree.Num())
			{
				...
				else
				{
					...
					if ((Views[0]->IsInstancedStereoPass() || Views[0]->bIsMobileMultiViewEnabled) && ViewIndex == 0)
					{
						...
						const int32 LastViewIndex = Views.Num() - 1;

						const FMatrix LeftEyeLocalViewProjForCulling  = GetLocalToWorld() * Views[0]->ViewMatrices.GetViewProjectionMatrix();
						const FMatrix RightEyeLocalViewProjForCulling = GetLocalToWorld() * Views[LastViewIndex]->ViewMatrices.GetViewProjectionMatrix();
						...
						FVector LeftBottom, LeftTop;
						FMath::IntersectPlanes3(LeftBottom, LeftEyeBounds.Planes[0], LeftEyeBounds.Planes[3], LeftEyeBounds.Planes[4]); // Left, Bottom, Far
						FMath::IntersectPlanes3(LeftTop, LeftEyeBounds.Planes[0], LeftEyeBounds.Planes[2], LeftEyeBounds.Planes[4]); // Left, Top, Far

						FVector RightBottom, RightTop;
						FMath::IntersectPlanes3(RightBottom, RightEyeBounds.Planes[1], RightEyeBounds.Planes[3], RightEyeBounds.Planes[4]); // Right, Bottom, Far
						FMath::IntersectPlanes3(RightTop, RightEyeBounds.Planes[1], RightEyeBounds.Planes[2], RightEyeBounds.Planes[4]); // Right, Top, Far

						FVector Origin = Views[0]->ViewMatrices.GetViewOrigin();

						FPlane TopPlane(Origin, LeftTop, RightTop);
						FPlane BottomPlane(Origin, RightBottom, LeftBottom);
						...
						InstanceParams.ViewFrustumLocal.Planes.Add(TopPlane);
						InstanceParams.ViewFrustumLocal.Planes.Add(BottomPlane);
