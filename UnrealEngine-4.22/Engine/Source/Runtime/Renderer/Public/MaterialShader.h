...
public:
	...
	FORCEINLINE_DEBUGGABLE void SetViewParameters(FRHICommandList& RHICmdList, const ShaderRHIParamRef ShaderRHI, const FSceneView& View, const TUniformBufferRef<FViewUniformShaderParameters>& ViewUniformBuffer)
	{
		...
		if (View.bShouldBindInstancedViewUB && View.Family->Views.Num() > 0)
		{
			...
			{
				const FSceneView& InstancedView2 = View.Family->GetStereoEyeView(eSSP_LEFT_EYE_SIDE);
				const auto& InstancedView2UniformBufferParameter = GetUniformBufferParameter<FInstancedView2UniformShaderParameters>();
				SetUniformBufferParameter(RHICmdList, ShaderRHI, InstancedView2UniformBufferParameter, InstancedView2.ViewUniformBuffer);
			}
			// FB Bulgakov End
		}
	}
