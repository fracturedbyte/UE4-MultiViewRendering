...
void FPersistentUniformBuffers::Initialize()
{
	...
	FInstancedView2UniformShaderParameters InstancedView2UniformBufferParameters;
	InstancedView2UniformBuffer = TUniformBufferRef<FInstancedView2UniformShaderParameters>::CreateUniformBufferImmediate(InstancedView2UniformBufferParameters, UniformBuffer_MultiFrame, EUniformBufferValidation::None);
	// FB Bulgakov End
	...
	InstancedCustomDepthView2UniformBuffer = TUniformBufferRef<FInstancedView2UniformShaderParameters>::CreateUniformBufferImmediate(InstancedView2UniformBufferParameters, UniformBuffer_MultiFrame, EUniformBufferValidation::None); 	// FB Bulgakov - Instanced Multi View Rendering
	...
	if (CachedView != &View)
	{
		...
		if ((View.IsInstancedStereoPass() || View.bIsMobileMultiViewEnabled) && View.Family->Views.Num() > 0)
		{
			...
			if (View.Family->Views.Num() > 2)
			{
				...
				InstancedView2UniformBuffer.UpdateUniformBufferImmediate(reinterpret_cast<FInstancedView2UniformShaderParameters&>(*InstancedView2.CachedViewUniformShaderParameters));
			}
		}
		// FB Bulgakov End
		else
