			...
			if (View.ShouldRenderView())
			{
				...
				if (CVarCustomDepthTemporalAAJitter.GetValueOnRenderThread() == 0 && View.AntiAliasingMethod == AAM_TemporalAA)
				{
					...
					if ((View.IsInstancedStereoPass() || View.bIsMobileMultiViewEnabled) && View.Family->Views.Num() > 0)
					{
						...
						if (View.Family->Views.Num() > 2)
						{
							const EStereoscopicPass StereoPassIndex = eSSP_LEFT_EYE_SIDE;

							const FViewInfo& InstancedView2 = static_cast<const FViewInfo&>(View.Family->GetStereoEyeView(StereoPassIndex));

							CustomDepthViewUniformBufferParameters = *InstancedView2.CachedViewUniformShaderParameters;
							ModifiedViewMatrices = InstancedView2.ViewMatrices;
							ModifiedViewMatrices.HackRemoveTemporalAAProjectionJitter();

							InstancedView2.SetupUniformBufferParameters(
								SceneContext,
								ModifiedViewMatrices,
								ModifiedViewMatrices,
								VolumeBounds,
							...
							Scene->UniformBuffers.InstancedCustomDepthView2UniformBuffer.UpdateUniformBufferImmediate(reinterpret_cast<FInstancedView2UniformShaderParameters&>(CustomDepthViewUniformBufferParameters));
						}
						// FB Bulgakov End
					}
				}
				else
				{
					...
					if ((View.IsInstancedStereoPass() || View.bIsMobileMultiViewEnabled) && View.Family->Views.Num() > 0)
					{
						...
						if (View.Family->Views.Num() > 2)
						{
							const FViewInfo& InstancedView2 = static_cast<const FViewInfo&>(View.Family->GetStereoEyeView(eSSP_LEFT_EYE_SIDE));
							Scene->UniformBuffers.InstancedCustomDepthView2UniformBuffer.UpdateUniformBufferImmediate(reinterpret_cast<FInstancedView2UniformShaderParameters&>(*InstancedView2.CachedViewUniformShaderParameters));
						}
						// FB Bulgakov End
					}
					else
