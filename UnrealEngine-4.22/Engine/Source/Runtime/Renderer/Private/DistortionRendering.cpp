		...
		if(DistortionRT)
		{
			...
			RHICmdList.BeginRenderPass(RPInfo, TEXT("RenderDistortion"));
			{
				for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
				{
					...
					DrawRenderState.SetInstancedView2UniformBuffer(Scene->UniformBuffers.InstancedView2UniformBuffer); // FB Bulgakov - Instanced Multi View Rendering
...
FMeshPassProcessor* CreateDistortionPassProcessor(const FScene* Scene, const FSceneView* InViewIfDynamicMeshCommand, FMeshPassDrawListContext* InDrawListContext)
{
	...
	DistortionPassState.SetInstancedView2UniformBuffer(Scene->UniformBuffers.InstancedView2UniformBuffer); // FB Bulgakov - Instanced Multi View Rendering
