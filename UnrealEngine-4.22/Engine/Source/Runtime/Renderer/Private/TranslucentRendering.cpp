			...
			if (DownsamplingScale < 1.f)
			{
				...
				if ((View.IsInstancedStereoPass() || View.bIsMobileMultiViewEnabled) && View.Family->Views.Num() > 0)
				{
					...
					if (View.Family->Views.Num() > 2)
					{
						...
						SetupDownsampledTranslucencyViewParameters(RHICmdList, InstancedView2, DownsampledTranslucencyViewParameters);
						Scene->UniformBuffers.InstancedView2UniformBuffer.UpdateUniformBufferImmediate(reinterpret_cast<FInstancedView2UniformShaderParameters&>(DownsampledTranslucencyViewParameters));
						DrawRenderState.SetInstancedView2UniformBuffer(Scene->UniformBuffers.InstancedView2UniformBuffer);
					}
					// FB Bulgakov End
				}
			}
